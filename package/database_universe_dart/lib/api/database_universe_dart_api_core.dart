...
// import 'package:database_universe_dart/api/extensions.dart';
// import 'package:database_universe_scheme/scheme/scheme/database_universe_pubspec_config.dart';
// import 'package:database_universe_scheme/scheme/scheme/pubspec_database_universe.dart';
// import 'package:database_universe_scheme/scheme/scheme/pubspec_database_universe_dependencies.dart';
...
enum DatabaseUniverseDartBuildType {
  debug,
  release,
}

class DatabaseUniverseDartApi {
  DatabaseUniverseDartApi();
  ...
  FutureOr<dynamic> create({
    ...
    // PubspecDatabaseUniverse pubspecDatabaseUniverse = PubspecDatabaseUniverse(yaml_code.clone());

    File file_guide = File(path.join(directory_project.path, "guide-database_universe.md"));

    await file_guide.writeAsString(guide_database_universe_markdown());
    ...
    if (!file_script_example.existsSync()) {
      // await file_script_example.writeAsString(script_database_universe_dart_native());
    }
    ...
    // PubspecDatabaseUniverse pubspecDatabaseUniverse_default = PubspecDatabaseUniverse.create(
    ...
    //   ],
    //   dependencies: PubspecDatabaseUniverseDependencies({
    //     "database_universe_dart": "any",
    //     "database_universe_dart_http_client": "any",
    //     "general_lib": "^0.0.34",
    //   }),
    //   database_universe: DatabaseUniversePubspecConfig.create(),
    // );

    // // update pubspec default
    // pubspecDatabaseUniverse.rawData.database_universe_dart_updateMapIfNotSameOrEmptyOrNull(
    //   data: pubspecDatabaseUniverse_default.toJson(),
    //   ignoreKeys: [
    //     "@type",
    //   ],
    // );
    // String yaml_documents_new = YamlWriter().write(pubspecDatabaseUniverse.toJson());
  ...
  FutureOr<int> run({
    required Directory directoryBase,
    required Directory? directoryOutputBuildDatabaseUniverse,
    required String? inputFileName,
    required DatabaseUniverseDartBuildType server_universeDartBuildType,
  }) async {
    if (Dart.isWeb) {
      return 1;
    }
  ...
  FutureOr<dynamic> build({
    ...
    Map yaml_code = (yaml.loadYaml(file_pubspec.readAsStringSync(), recover: true) as Map);
    // PubspecDatabaseUniverse pubspec_server_universe = PubspecDatabaseUniverse(yaml_code.clone());
  ...
  static String guide_database_universe_markdown() {
    return r"""
